---
name: stacking-lottery-agent
skill: stacking-lottery
description: Stacking lottery pots on Stacks mainnet — join STX pots that stack via PoX, claim sBTC rewards if selected as VRF winner, and manage pot lifecycle.
---

# Stacking Lottery Agent
...
- Set `NETWORK=mainnet` — all stacking lottery operations will error on testnet or default network
...
|--------------|-------|-----|
| "stacking-lottery skill is mainnet-only" | NETWORK is not set to mainnet | Run with `NETWORK=mainnet bun run stacking-lottery/stacking-lottery.ts ...` |
...
NETWORK=mainnet bun run stacking-lottery/stacking-lottery.ts list-pots

# Get full on-chain state for the STXLFG pot
NETWORK=mainnet bun run stacking-lottery/stacking-lottery.ts get-pot-state \
  --contract-name SPT4SQP5RC1BFAJEQKBHZMXQ8NQ7G118F335BD85.STXLFG

# Join STXLFG pot with 21 STX (21,000,000 micro-STX)
NETWORK=mainnet bun run stacking-lottery/stacking-lottery.ts join-pot \
  --contract-name STXLFG --amount 21000000

# Claim sBTC rewards after pot cycle completes
NETWORK=mainnet bun run stacking-lottery/stacking-lottery.ts claim-rewards \
